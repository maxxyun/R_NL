...
pop_move<-read.csv(file="./R 기초/D2/data/popMove.csv")

#맥의 경우 UTF-8로 기본으로 읽기 때문에 마소의 기본인 euc-kr로 읽으라고 따로 설정해야 함.
#pop_move<-read.csv(file="./R 기초/D2/data/popMove.csv",fileEncoding="euc-kr")
...
pm<-read.csv(file="./R 기초/D2/data/popMove.csv",
             skip=2,
             header = FALSE,
...
write.csv(pop_move,file = "./R 기초/D2/output/popMove.csv",
          row.names = FALSE
         )                              #열에 숫자가 들어가는데 끔.
